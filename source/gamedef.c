...
const char *keyw[] = {
    ...
    "action",                   // 7  defines an action if used outside a state or actor, otherwise triCASUL_WEAPONggers actor to perform action
...
//            initprintf("%s:%d: rewriting empty braces '{ }' as 'nullop' from right\n",g_szScriptFileName,g_lineNumber);
    ...
    case CON_GAMESTARTUP: {
        ...
        for (j = 0; j < 34; j++) {
            ...
            if (C_GetKeyword() != -1) {
                initprintf("RR style CON files detected.\n");
                break;
            } else {
                g_scriptVersion = 14;
                initprintf("RA style CON files detected.\n");
            }
        ...
        CROSSBOWBLASTRADIUS
        TNTRADIUS
        ALIENARMGUNBLASTRADIUS
        POWDERKEGBLASTRADIUS
        MORTERBLASTRADIUS
        ...
        */
        ...
        g_ALIENARMGUNBlastRadius = params[j++];
        g_tripbombBlastRadius = params[j++];
        g_morterBlastRadius = params[j++];
        g_bouncemineBlastRadius = params[j++];
        g_seenineBlastRadius = params[j++];
        g_player[0].ps->max_ammo_amount[CASUL_WEAPON] = params[j++];
        g_player[0].ps->max_ammo_amount[SHOTGUN_WEAPON] = params[j++];
        g_player[0].ps->max_ammo_amount[RIFLE_WEAPON] = params[j++];
        g_player[0].ps->max_ammo_amount[CROSSBOW_WEAPON] = params[j++];
        g_player[0].ps->max_ammo_amount[DYNAMITE_WEAPON] = params[j++];
        g_player[0].ps->max_ammo_amount[THROWSAW_WEAPON] = params[j++];
        g_player[0].ps->max_ammo_amount[ALIENARMGUN_WEAPON] = params[j++];
        g_player[0].ps->max_ammo_amount[POWDERKEG_WEAPON] = params[j++];
        g_player[0].ps->max_ammo_amount[TEATGUN_WEAPON] = params[j++];
        g_player[0].ps->max_ammo_amount[BUZSAW_WEAPON] = params[j++];
        g_player[0].ps->max_ammo_amount[BOWLINGBALL_WEAPON] = params[j++];
        g_damageCameras = params[j++];
        g_numBUZSAWBounces = params[j++];
        g_BUZSAWrSelfDamage = params[j++];
        ...
        if (g_scriptVersion == 14) {
            g_player[0].ps->max_ammo_amount[MOTORCYCLE_WEAPON] = params[j++];
            g_player[0].ps->max_ammo_amount[BOAT_WEAPON] = params[j++];
            g_player[0].ps->max_ammo_amount[CHICKEN_WEAPON] = params[j++];
        }
    }
    return 0;
    }
...
static void C_InitProjectiles(void) {
    ...
    struct {
        ...
        1, 100, -1, -1, 2048, 0, 0,
        SMALLSMOKE, -1, -1, 600, BULLETHOLE, -1, 0, 0, 32, 448, g_numBUZSAWBounces, PIPEBOMB_BOUNCE, 1,
...
void C_Compile(const char *filenam) {
    ...
    if (fp == -1) { // JBF: was 0
        ...
        if (g_loadFromGroupOnly == 1) {
            Bsprintf(tempbuf,"'%s' missing CON files, reinstall the game.",duke3dgrp);
        } else if (numgroupfiles == 0) {
            Bsprintf(tempbuf,"Game data was not found.  A copy of '%s' or other compatible data is needed to run eRampage.\n"
                     "eRampage will now close.",
                     duke3dgrp);
