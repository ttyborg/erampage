...
static void __fastcall X_GetPlayer(register int32_t lVar1, register int32_t lLabelID, register int32_t lVar2, int32_t lParm2) {
    ...
    case PLAYER_HOLODUKE_AMOUNT:
        Gv_SetVar(lVar2, g_player[iPlayer].ps->inv_amount[GET_HOLODUKE], vm.g_i, vm.g_p);
        return;
    ...
    case PLAYER_FIRSTAID_AMOUNT:
        Gv_SetVar(lVar2, g_player[iPlayer].ps->inv_amount[GET_FIRSTAID], vm.g_i, vm.g_p);
        return;
    ...
    case PLAYER_HEAT_AMOUNT:
        Gv_SetVar(lVar2, g_player[iPlayer].ps->inv_amount[GET_HEATS], vm.g_i, vm.g_p);
        return;
    ...
    case PLAYER_SCUBA_AMOUNT:
        Gv_SetVar(lVar2, g_player[iPlayer].ps->inv_amount[GET_SCUBA], vm.g_i, vm.g_p);
        return;
    case PLAYER_JETPACK_AMOUNT:
        Gv_SetVar(lVar2, g_player[iPlayer].ps->inv_amount[GET_JETPACK], vm.g_i, vm.g_p);
        return;
    case PLAYER_STEROIDS_AMOUNT:
        Gv_SetVar(lVar2, g_player[iPlayer].ps->inv_amount[GET_STEROIDS], vm.g_i, vm.g_p);
        return;
    case PLAYER_SHIELD_AMOUNT:
        Gv_SetVar(lVar2, g_player[iPlayer].ps->inv_amount[GET_SHIELD], vm.g_i, vm.g_p);
        return;
    ...
    case PLAYER_BOOT_AMOUNT:
        Gv_SetVar(lVar2, g_player[iPlayer].ps->inv_amount[GET_BOOTS], vm.g_i, vm.g_p);
        return;
...
static void __fastcall X_SetPlayer(int32_t lVar1, int32_t lLabelID, int32_t lVar2, int32_t lParm2) {
    ...
    switch (lLabelID) {
    case PLAYER_ZOOM:
        g_player[iPlayer].ps->zoom=lVar1;
        return;
    case PLAYER_EXITX:
        g_player[iPlayer].ps->exitx=lVar1;
        return;
    case PLAYER_EXITY:
        g_player[iPlayer].ps->exity=lVar1;
        return;
    case PLAYER_LOOGIEX:
        g_player[iPlayer].ps->loogiex[lParm2]=lVar1;
        return;
    case PLAYER_LOOGIEY:
        g_player[iPlayer].ps->loogiey[lParm2]=lVar1;
        return;
    case PLAYER_NUMLOOGS:
        g_player[iPlayer].ps->numloogs=lVar1;
        return;
    case PLAYER_LOOGCNT:
        g_player[iPlayer].ps->loogcnt=lVar1;
        return;
    case PLAYER_POSX:
        g_player[iPlayer].ps->posx=lVar1;
        return;
    case PLAYER_POSY:
        g_player[iPlayer].ps->posy=lVar1;
        return;
    case PLAYER_POSZ:
        g_player[iPlayer].ps->posz=lVar1;
        return;
    case PLAYER_HORIZ:
        g_player[iPlayer].ps->horiz=lVar1;
        return;
    case PLAYER_OHORIZ:
        g_player[iPlayer].ps->ohoriz=lVar1;
        return;
    case PLAYER_OHORIZOFF:
        g_player[iPlayer].ps->ohorizoff=lVar1;
        return;
    case PLAYER_INVDISPTIME:
        g_player[iPlayer].ps->invdisptime=lVar1;
        return;
    case PLAYER_BOBPOSX:
        g_player[iPlayer].ps->bobposx=lVar1;
        return;
    case PLAYER_BOBPOSY:
        g_player[iPlayer].ps->bobposy=lVar1;
        return;
    case PLAYER_OPOSX:
        g_player[iPlayer].ps->oposx=lVar1;
        return;
    case PLAYER_OPOSY:
        g_player[iPlayer].ps->oposy=lVar1;
        return;
    case PLAYER_OPOSZ:
        g_player[iPlayer].ps->oposz=lVar1;
        return;
    case PLAYER_PYOFF:
        g_player[iPlayer].ps->pyoff=lVar1;
        return;
    case PLAYER_OPYOFF:
        g_player[iPlayer].ps->opyoff=lVar1;
        return;
    case PLAYER_POSXV:
        g_player[iPlayer].ps->posxv=lVar1;
        return;
    case PLAYER_POSYV:
        g_player[iPlayer].ps->posyv=lVar1;
        return;
    case PLAYER_POSZV:
        g_player[iPlayer].ps->poszv=lVar1;
        return;
    case PLAYER_LAST_PISSED_TIME:
        g_player[iPlayer].ps->last_pissed_time=lVar1;
        return;
    case PLAYER_TRUEFZ:
        g_player[iPlayer].ps->truefz=lVar1;
        return;
    case PLAYER_TRUECZ:
        g_player[iPlayer].ps->truecz=lVar1;
        return;
    case PLAYER_PLAYER_PAR:
        g_player[iPlayer].ps->player_par=lVar1;
        return;
    case PLAYER_VISIBILITY:
        g_player[iPlayer].ps->visibility=lVar1;
        return;
    case PLAYER_BOBCOUNTER:
        g_player[iPlayer].ps->bobcounter=lVar1;
        return;
    case PLAYER_WEAPON_SWAY:
        g_player[iPlayer].ps->weapon_sway=lVar1;
        return;
    case PLAYER_PALS_TIME:
        g_player[iPlayer].ps->pals_time=lVar1;
        return;
    case PLAYER_RANDOMFLAMEX:
        g_player[iPlayer].ps->randomflamex=lVar1;
        return;
    case PLAYER_CRACK_TIME:
        g_player[iPlayer].ps->crack_time=lVar1;
        return;
    case PLAYER_AIM_MODE:
        g_player[iPlayer].ps->aim_mode=lVar1;
        return;
    case PLAYER_ANG:
        g_player[iPlayer].ps->ang=lVar1;
        return;
    case PLAYER_OANG:
        g_player[iPlayer].ps->oang=lVar1;
        return;
    case PLAYER_ANGVEL:
        g_player[iPlayer].ps->angvel=lVar1;
        return;
    case PLAYER_CURSECTNUM:
        g_player[iPlayer].ps->cursectnum=lVar1;
        return;
    case PLAYER_LOOK_ANG:
        g_player[iPlayer].ps->look_ang=lVar1;
        return;
    case PLAYER_LAST_EXTRA:
        g_player[iPlayer].ps->last_extra=lVar1;
        return;
    case PLAYER_SUBWEAPON:
        g_player[iPlayer].ps->subweapon=lVar1;
        return;
    case PLAYER_AMMO_AMOUNT:
        g_player[iPlayer].ps->ammo_amount[lParm2]=lVar1;
        return;
    case PLAYER_WACKEDBYACTOR:
        g_player[iPlayer].ps->wackedbyactor=lVar1;
        return;
    case PLAYER_FRAG:
        g_player[iPlayer].ps->frag=lVar1;
        return;
    case PLAYER_FRAGGEDSELF:
        g_player[iPlayer].ps->fraggedself=lVar1;
        return;
    case PLAYER_CURR_WEAPON:
        g_player[iPlayer].ps->curr_weapon=lVar1;
        return;
    case PLAYER_LAST_WEAPON:
        g_player[iPlayer].ps->last_weapon=lVar1;
        return;
    case PLAYER_TIPINCS:
        g_player[iPlayer].ps->tipincs=lVar1;
        return;
    case PLAYER_HORIZOFF:
        g_player[iPlayer].ps->horizoff=lVar1;
        return;
    case PLAYER_WANTWEAPONFIRE:
        g_player[iPlayer].ps->wantweaponfire=lVar1;
        return;
    case PLAYER_HOLODUKE_AMOUNT:
        g_player[iPlayer].ps->inv_amount[GET_HOLODUKE]=lVar1;
        return;
    case PLAYER_NEWOWNER:
        g_player[iPlayer].ps->newowner=lVar1;
        return;
    case PLAYER_HURT_DELAY:
        g_player[iPlayer].ps->hurt_delay=lVar1;
        return;
    case PLAYER_HBOMB_HOLD_DELAY:
        g_player[iPlayer].ps->hbomb_hold_delay=lVar1;
        return;
    case PLAYER_JUMPING_COUNTER:
        g_player[iPlayer].ps->jumping_counter=lVar1;
        return;
    case PLAYER_AIRLEFT:
        g_player[iPlayer].ps->airleft=lVar1;
        return;
    case PLAYER_KNEE_INCS:
        g_player[iPlayer].ps->knee_incs=lVar1;
        return;
    case PLAYER_ACCESS_INCS:
        g_player[iPlayer].ps->access_incs=lVar1;
        return;
    case PLAYER_FTA:
        g_player[iPlayer].ps->fta=lVar1;
        return;
    case PLAYER_FTQ:
        g_player[iPlayer].ps->ftq=lVar1;
        return;
    case PLAYER_ACCESS_WALLNUM:
        g_player[iPlayer].ps->access_wallnum=lVar1;
        return;
    case PLAYER_ACCESS_SPRITENUM:
        g_player[iPlayer].ps->access_spritenum=lVar1;
        return;
    case PLAYER_KICKBACK_PIC:
        g_player[iPlayer].ps->kickback_pic=lVar1;
        return;
    case PLAYER_GOT_ACCESS:
        g_player[iPlayer].ps->got_access=lVar1;
        return;
    case PLAYER_WEAPON_ANG:
        g_player[iPlayer].ps->weapon_ang=lVar1;
        return;
    case PLAYER_FIRSTAID_AMOUNT:
        g_player[iPlayer].ps->inv_amount[GET_FIRSTAID]=lVar1;
        return;
    case PLAYER_SOMETHINGONPLAYER:
        g_player[iPlayer].ps->somethingonplayer=lVar1;
        return;
    case PLAYER_ON_CRANE:
        g_player[iPlayer].ps->on_crane=lVar1;
        return;
    case PLAYER_I:
        g_player[iPlayer].ps->i=lVar1;
        return;
    case PLAYER_ONE_PARALLAX_SECTNUM:
        g_player[iPlayer].ps->one_parallax_sectnum=lVar1;
        return;
    case PLAYER_OVER_SHOULDER_ON:
        g_player[iPlayer].ps->over_shoulder_on=lVar1;
        return;
    case PLAYER_RANDOM_CLUB_FRAME:
        g_player[iPlayer].ps->random_club_frame=lVar1;
        return;
    case PLAYER_FIST_INCS:
        g_player[iPlayer].ps->fist_incs=lVar1;
        return;
    case PLAYER_ONE_EIGHTY_COUNT:
        g_player[iPlayer].ps->one_eighty_count=lVar1;
        return;
    case PLAYER_CHEAT_PHASE:
        g_player[iPlayer].ps->cheat_phase=lVar1;
        return;
    case PLAYER_DUMMYPLAYERSPRITE:
        g_player[iPlayer].ps->dummyplayersprite=lVar1;
        return;
    case PLAYER_EXTRA_EXTRA8:
        g_player[iPlayer].ps->extra_extra8=lVar1;
        return;
    case PLAYER_QUICK_KICK:
        g_player[iPlayer].ps->quick_kick=lVar1;
        return;
    case PLAYER_HEAT_AMOUNT:
        g_player[iPlayer].ps->inv_amount[GET_HEATS]=lVar1;
        return;
    case PLAYER_ACTORSQU:
        g_player[iPlayer].ps->actorsqu=lVar1;
        return;
    case PLAYER_TIMEBEFOREEXIT:
        g_player[iPlayer].ps->timebeforeexit=lVar1;
        return;
    case PLAYER_CUSTOMEXITSOUND:
        g_player[iPlayer].ps->customexitsound=lVar1;
        return;
    case PLAYER_WEAPRECS:
        g_player[iPlayer].ps->weaprecs[15]=lVar1;
        return;
    case PLAYER_WEAPRECCNT:
        g_player[iPlayer].ps->weapreccnt=lVar1;
        return;
    case PLAYER_INTERFACE_TOGGLE_FLAG:
        g_player[iPlayer].ps->interface_toggle_flag=lVar1;
        return;
    case PLAYER_ROTSCRNANG:
        g_player[iPlayer].ps->rotscrnang=lVar1;
        return;
    case PLAYER_DEAD_FLAG:
        g_player[iPlayer].ps->dead_flag=lVar1;
        return;
    case PLAYER_SHOW_EMPTY_WEAPON:
        g_player[iPlayer].ps->show_empty_weapon=lVar1;
        return;
    case PLAYER_SCUBA_AMOUNT:
        g_player[iPlayer].ps->inv_amount[GET_SCUBA]=lVar1;
        return;
    case PLAYER_JETPACK_AMOUNT:
        g_player[iPlayer].ps->inv_amount[GET_JETPACK]=lVar1;
        return;
    case PLAYER_STEROIDS_AMOUNT:
        g_player[iPlayer].ps->inv_amount[GET_STEROIDS]=lVar1;
        return;
    case PLAYER_SHIELD_AMOUNT:
        g_player[iPlayer].ps->inv_amount[GET_SHIELD]=lVar1;
        return;
    case PLAYER_HOLODUKE_ON:
        g_player[iPlayer].ps->holoduke_on=lVar1;
        return;
    case PLAYER_PYCOUNT:
        g_player[iPlayer].ps->pycount=lVar1;
        return;
    case PLAYER_WEAPON_POS:
        g_player[iPlayer].ps->weapon_pos=lVar1;
        return;
    case PLAYER_FRAG_PS:
        g_player[iPlayer].ps->frag_ps=lVar1;
        return;
    case PLAYER_TRANSPORTER_HOLD:
        g_player[iPlayer].ps->transporter_hold=lVar1;
        return;
    case PLAYER_LAST_FULL_WEAPON:
        g_player[iPlayer].ps->last_full_weapon=lVar1;
        return;
    case PLAYER_FOOTPRINTSHADE:
        g_player[iPlayer].ps->footprintshade=lVar1;
        return;
    case PLAYER_BOOT_AMOUNT:
        g_player[iPlayer].ps->inv_amount[GET_BOOTS]=lVar1;
        return;
    case PLAYER_SCREAM_VOICE:
        g_player[iPlayer].ps->scream_voice=lVar1;
        return;
    case PLAYER_GM:
        g_player[iPlayer].ps->gm=lVar1;
        return;
    case PLAYER_ON_WARPING_SECTOR:
        g_player[iPlayer].ps->on_warping_sector=lVar1;
        return;
    case PLAYER_FOOTPRINTCOUNT:
        g_player[iPlayer].ps->footprintcount=lVar1;
        return;
    case PLAYER_HBOMB_ON:
        g_player[iPlayer].ps->hbomb_on=lVar1;
        return;
    case PLAYER_JUMPING_TOGGLE:
        g_player[iPlayer].ps->jumping_toggle=lVar1;
        return;
    case PLAYER_RAPID_FIRE_HOLD:
        g_player[iPlayer].ps->rapid_fire_hold=lVar1;
        return;
    case PLAYER_ON_GROUND:
        g_player[iPlayer].ps->on_ground=lVar1;
        return;
    case PLAYER_INVEN_ICON:
        g_player[iPlayer].ps->inven_icon=lVar1;
        return;
    case PLAYER_BUTTONPALETTE:
        g_player[iPlayer].ps->buttonpalette=lVar1;
        return;
    case PLAYER_JETPACK_ON:
        g_player[iPlayer].ps->jetpack_on=lVar1;
        return;
    case PLAYER_SPRITEBRIDGE:
        g_player[iPlayer].ps->spritebridge=lVar1;
        return;
    case PLAYER_LASTRANDOMSPOT:
        g_player[iPlayer].ps->lastrandomspot=lVar1;
        return;
    case PLAYER_SCUBA_ON:
        g_player[iPlayer].ps->scuba_on=lVar1;
        return;
    case PLAYER_FOOTPRINTPAL:
        g_player[iPlayer].ps->footprintpal=lVar1;
        return;
    case PLAYER_HEAT_ON:
        ...
        return;
    case PLAYER_HOLSTER_WEAPON:
        g_player[iPlayer].ps->holster_weapon=lVar1;
        return;
    case PLAYER_FALLING_COUNTER:
        g_player[iPlayer].ps->falling_counter=lVar1;
        return;
    case PLAYER_GOTWEAPON:
        g_player[iPlayer].ps->gotweapon[lParm2]=lVar1;
        return;
    case PLAYER_REFRESH_INVENTORY:
        g_player[iPlayer].ps->refresh_inventory=lVar1;
        return;
    case PLAYER_TOGGLE_KEY_FLAG:
        g_player[iPlayer].ps->toggle_key_flag=lVar1;
        return;
    case PLAYER_KNUCKLE_INCS:
        g_player[iPlayer].ps->knuckle_incs=lVar1;
        return;
    case PLAYER_WALKING_SND_TOGGLE:
        g_player[iPlayer].ps->walking_snd_toggle=lVar1;
        return;
    case PLAYER_PALOOKUP:
        g_player[iPlayer].ps->palookup=lVar1;
        return;
    case PLAYER_HARD_LANDING:
        g_player[iPlayer].ps->hard_landing=lVar1;
        return;
    case PLAYER_MAX_SECRET_ROOMS:
        g_player[iPlayer].ps->max_secret_rooms=lVar1;
        return;
    case PLAYER_SECRET_ROOMS:
        g_player[iPlayer].ps->secret_rooms=lVar1;
        return;
    case PLAYER_PALS:
        g_player[iPlayer].ps->pals[lParm2]=lVar1;
        return;
    case PLAYER_MAX_ACTORS_KILLED:
        g_player[iPlayer].ps->max_actors_killed=lVar1;
        return;
    case PLAYER_ACTORS_KILLED:
        g_player[iPlayer].ps->actors_killed=lVar1;
        return;
    case PLAYER_RETURN_TO_CENTER:
        g_player[iPlayer].ps->return_to_center=lVar1;
        return;
    case PLAYER_RUNSPEED:
        g_player[iPlayer].ps->runspeed=lVar1;
        return;
    case PLAYER_SBS:
        g_player[iPlayer].ps->sbs=lVar1;
        return;
    case PLAYER_RELOADING:
        g_player[iPlayer].ps->reloading=lVar1;
        return;
    case PLAYER_AUTO_AIM:
        g_player[iPlayer].ps->auto_aim=lVar1;
        return;
    case PLAYER_MOVEMENT_LOCK:
        g_player[iPlayer].ps->movement_lock=lVar1;
        return;
    case PLAYER_SOUND_PITCH:
        g_player[iPlayer].ps->sound_pitch=lVar1;
        return;
    case PLAYER_WEAPONSWITCH:
        g_player[iPlayer].ps->weaponswitch=lVar1;
        return;
    case PLAYER_TEAM:
        g_player[iPlayer].ps->team=lVar1;
        return;
    case PLAYER_MAX_PLAYER_HEALTH:
        g_player[iPlayer].ps->max_player_health = lVar1;
        return;
    case PLAYER_MAX_SHIELD_AMOUNT:
        g_player[iPlayer].ps->max_shield_amount = lVar1;
        return;
    case PLAYER_MAX_AMMO_AMOUNT:
        g_player[iPlayer].ps->max_ammo_amount[lParm2]=lVar1;
        return;
    case PLAYER_LAST_QUICK_KICK:
...
static int32_t __fastcall X_AccessSpriteX(int32_t iActor, int32_t lLabelID, int32_t lParm2) {
    ...
    switch (lLabelID) {
    case ACTOR_X:
        return sprite[iActor].x;
    case ACTOR_Y:
        return sprite[iActor].y;
    case ACTOR_Z:
        return sprite[iActor].z;
    case ACTOR_CSTAT:
        return sprite[iActor].cstat;
    case ACTOR_PICNUM:
        return sprite[iActor].picnum;
    case ACTOR_SHADE:
        return sprite[iActor].shade;
    case ACTOR_PAL:
        return sprite[iActor].pal;
    case ACTOR_CLIPDIST:
        return sprite[iActor].clipdist;
    case ACTOR_DETAIL:
        return sprite[iActor].filler;
    case ACTOR_XREPEAT:
        return sprite[iActor].xrepeat;
    case ACTOR_YREPEAT:
        return sprite[iActor].yrepeat;
    case ACTOR_XOFFSET:
        return sprite[iActor].xoffset;
    case ACTOR_YOFFSET:
        return sprite[iActor].yoffset;
    case ACTOR_SECTNUM:
        return sprite[iActor].sectnum;
    case ACTOR_STATNUM:
        return sprite[iActor].statnum;
    case ACTOR_ANG:
        return sprite[iActor].ang;
    case ACTOR_OWNER:
        return sprite[iActor].owner;
    case ACTOR_XVEL:
        return sprite[iActor].xvel;
    case ACTOR_YVEL:
        return sprite[iActor].yvel;
    case ACTOR_ZVEL:
        return sprite[iActor].zvel;
    case ACTOR_LOTAG:
        return sprite[iActor].lotag;
    case ACTOR_HITAG:
        return sprite[iActor].hitag;
    case ACTOR_EXTRA:
        return sprite[iActor].extra;
    case ACTOR_HTCGG:
        return ActorExtra[iActor].cgg;
    case ACTOR_HTPICNUM :
        return ActorExtra[iActor].picnum;
    case ACTOR_HTANG:
        return ActorExtra[iActor].ang;
    case ACTOR_HTEXTRA:
        return ActorExtra[iActor].extra;
    case ACTOR_HTOWNER:
        return ActorExtra[iActor].owner;
    case ACTOR_HTMOVFLAG:
        return ActorExtra[iActor].movflag;
    case ACTOR_HTTEMPANG:
        return ActorExtra[iActor].tempang;
    case ACTOR_HTACTORSTAYPUT:
        return ActorExtra[iActor].actorstayput;
    case ACTOR_HTDISPICNUM:
        return ActorExtra[iActor].dispicnum;
    case ACTOR_HTTIMETOSLEEP:
        return ActorExtra[iActor].timetosleep;
    case ACTOR_HTFLOORZ:
        return ActorExtra[iActor].floorz;
    case ACTOR_HTCEILINGZ:
        return ActorExtra[iActor].ceilingz;
    case ACTOR_HTLASTVX:
        return ActorExtra[iActor].lastvx;
    case ACTOR_HTLASTVY:
        return ActorExtra[iActor].lastvy;
    case ACTOR_HTBPOSX:
        return ActorExtra[iActor].bposx;
    case ACTOR_HTBPOSY:
        return ActorExtra[iActor].bposy;
    case ACTOR_HTBPOSZ:
        return ActorExtra[iActor].bposz;
    case ACTOR_HTG_T:
        return ActorExtra[iActor].temp_data[lParm2];
    case ACTOR_ANGOFF:
        return spriteext[iActor].angoff;
    case ACTOR_PITCH:
        return spriteext[iActor].pitch;
    case ACTOR_ROLL:
        return spriteext[iActor].roll;
    case ACTOR_MDXOFF:
        return spriteext[iActor].xoff;
    case ACTOR_MDYOFF:
        return spriteext[iActor].yoff;
    case ACTOR_MDZOFF:
        return spriteext[iActor].zoff;
    case ACTOR_MDFLAGS:
        return spriteext[iActor].flags;
    case ACTOR_XPANNING:
        return spriteext[iActor].xpanning;
    case ACTOR_YPANNING:
        return spriteext[iActor].ypanning;
    case ACTOR_HTFLAGS:
        return ActorExtra[iActor].flags;
    case ACTOR_ALPHA:
        return (uint8_t)(spriteext[iActor].alpha*255.0f);
    default:
        return -1;
    }
...
static int32_t __fastcall X_AccessSectorX(int32_t iSector, int32_t lLabelID) {
    switch (lLabelID) {
    case SECTOR_WALLPTR:
        return sector[iSector].wallptr;
    case SECTOR_WALLNUM:
        return sector[iSector].wallnum;
    case SECTOR_CEILINGZ:
        return sector[iSector].ceilingz;
    case SECTOR_FLOORZ:
        return sector[iSector].floorz;
    case SECTOR_CEILINGSTAT:
        return sector[iSector].ceilingstat;
    case SECTOR_FLOORSTAT:
        return sector[iSector].floorstat;
    case SECTOR_CEILINGPICNUM:
        return sector[iSector].ceilingpicnum;
    case SECTOR_CEILINGSLOPE:
        return sector[iSector].ceilingheinum;
    case SECTOR_CEILINGSHADE:
        return sector[iSector].ceilingshade;
    case SECTOR_CEILINGPAL:
        return sector[iSector].ceilingpal;
    case SECTOR_CEILINGXPANNING:
        return sector[iSector].ceilingxpanning;
    case SECTOR_CEILINGYPANNING:
        return sector[iSector].ceilingypanning;
    case SECTOR_FLOORPICNUM:
        return sector[iSector].floorpicnum;
    case SECTOR_FLOORSLOPE:
        return sector[iSector].floorheinum;
    case SECTOR_FLOORSHADE:
        return sector[iSector].floorshade;
    case SECTOR_FLOORPAL:
        return sector[iSector].floorpal;
    case SECTOR_FLOORXPANNING:
        return sector[iSector].floorxpanning;
    case SECTOR_FLOORYPANNING:
        return sector[iSector].floorypanning;
    case SECTOR_VISIBILITY:
        return sector[iSector].visibility;
    case SECTOR_ALIGNTO:
        return sector[iSector].filler;
    case SECTOR_LOTAG:
        return sector[iSector].lotag;
    case SECTOR_HITAG:
        return sector[iSector].hitag;
    case SECTOR_EXTRA:
        return sector[iSector].extra;
    default:
        return -1;
    }
}
