...
void GAME_drawosdchar(int32_t x, int32_t y, char ch, int32_t shade, int32_t pal) {
    ...
    rotatesprite(((x<<3)+x)<<16, (y<<3)<<16, 65536l/2, 0, ac, shade, pal, 8|16, 0, 0, xdim-1, ydim-1);
    usehightile = ht;
}
...
void GAME_drawosdstr(int32_t x, int32_t y, char *ch, int32_t len, int32_t shade, int32_t pal) {
    ...
    if (ch > ptr && ch < (ptr + TEXTSIZE)) {
        do {
            ...
            rotatesprite(x<<16, (y<<3)<<16, 32768, 0, ac, (*(ch-ptr+fmt)&~0x1F)>>4,
    ...
    do {
        ...
        rotatesprite(x<<16, (y<<3)<<16, 32768, 0, ac, shade, pal, 8|16, 0, 0, xdim-1, ydim-1);
        x += OSDCHAR_WIDTH+1;
        ch++;
    } while (--len);
...
void GAME_drawosdcursor(int32_t x, int32_t y, int32_t type, int32_t lastkeypress) {
    ...
    if (!((GetTime()-lastkeypress) & 0x40l))
        rotatesprite(((x<<3)+x)<<16, ((y<<3)+(type?-1:2))<<16, 65536L/2, 0, ac, 0, 8, 8|16, 0, 0, xdim-1, ydim-1);
}
...
//#define BGTILE 1156
#define BGTILE 1936
#define BGTILE_SIZEX 256
#define BGTILE_SIZEY 256
...
void GAME_onshowosd(int32_t shown) {
    ...
    if (!tilesizx[BGTILE] || !tilesizy[BGTILE]) {
        ...
        j = 15;
        while ((j > 1) && (pow2long[j] > BGTILE_SIZEX)) j--;
        picsiz[BGTILE] = ((uint8_t)j);
        j = 15;
        while ((j > 1) && (pow2long[j] > BGTILE_SIZEY)) j--;
...
//            rotatesprite(x*xsiz<<16,((daydim-ydim)+(y*ysiz))<<16,65536L,0,BGTILE,SHADE,PALETTE,bits,0,0,xdim,daydim);
            rotatesprite(x*xsiz<<16,y*ysiz<<16,65536L/2,0,BGTILE,SHADE,PALETTE,bits,0,0,xdim,daydim);
    ...
    for (x=tx2; x>=0; x--)
        rotatesprite(x*xsiz<<16,(daydim+ysiz+1)<<16,65536L/2,1536,BORDTILE,SHADE-12,PALETTE,BITS,0,0,xdim,daydim+ysiz+1);
}
