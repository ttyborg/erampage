...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, LPSTR lpCmdLine, int nCmdShow) {
    ...
    if (szFileName[0] != '\0' && CmdLine[0] != '\0') Bsprintf(sCmdLine,"erampage.exe %s -rmnet %s",CmdLine,szFileName);
    else if (CmdLine[0] != '\0') Bsprintf(sCmdLine,"erampage.exe %s",CmdLine);
    else Bsprintf(sCmdLine,"erampage.exe");
