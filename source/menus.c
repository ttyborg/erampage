...
static int32_t probe_(int32_t type,int32_t x,int32_t y,int32_t i,int32_t n) {
    ...
    if (centre) {
        ...
        rotatesprite(((320>>1)+(centre>>1)+70)<<16,(y+(probey*i)-4)<<16,8192L>>type,0,SPINNINGNUKEICON+15-((15+(totalclock>>3))%16),sh,0,10,0,0,xdim-1,ydim-1);
        rotatesprite(((320>>1)-(centre>>1)-70)<<16,(y+(probey*i)-4)<<16,8192L>>type,0,SPINNINGNUKEICON+((totalclock>>3)%16),sh,0,10,0,0,xdim-1,ydim-1);
    } else

        rotatesprite((x<<16)-((tilesizx[BIGFNTCURSOR]-4)<<(16-type)),(y+(probey*i)-(4>>type))<<16,8192L>>type,0,SPINNINGNUKEICON+(((totalclock>>3))%16),sh,0,10,0,0,xdim-1,ydim-1);
...
//    usehightile = (ht && r_downsize < 3);
    while (*t) {
        ...
        rotatesprite(x<<16,y<<16,65536L/2,0,ac,s,p,bits,0,0,xdim-1,ydim-1);
...
static void _bar(int32_t type, int32_t x,int32_t y,int32_t *p,int32_t dainc,int32_t damodify,int32_t s, int32_t pa, int32_t min, int32_t max) {
    ...
    xloc = *p;
    rotatesprite((x<<16)+(36<<(16-type)),(y<<16)-(3<<(16-type)),65536L>>type,0,SLIDEBAR,s,pa,10,0,0,xdim-1,ydim-1);
    if (rev == 0)
        rotatesprite((x<<16)+((scale(64,xloc-min,max-min)+1)<<(16-type)),(y<<16)+(1<<(16-type)),65536L>>type,0,SLIDEBARBULLET,s,pa,10,0,0,xdim-1,ydim-1);
    else
        rotatesprite((x<<16)+((65-scale(64,xloc-min,max-min))<<(16-type)),(y<<16)+(1<<(16-type)),65536L>>type,0,SLIDEBARBULLET,s,pa,10,0,0,xdim-1,ydim-1);
}
...
void M_DisplayMenus(void) {
    ...
    case 998:
        ...
        if (!VOLUMEALL || !GAME_RA) {
            ...
            menutext(c,24,0,0,g_currentMenu == 998 ? "ABOUT ERAMPAGE" : "CREDITS");
        ...
        case 993:   // JBF 20031220
            ...
            menutext(160,24,0,0,"ABOUT ERAMPAGE");

cheat_for_port_credits:
            if (g_scriptVersion == 13) l = (-2);
            mgametext(160,34-l,"REDNECK-RELATED PROGRAMMING",0,2+8+16);
            p = "Dmitry \"dimag0g\" Grigoryev";
            minitext(161-(Bstrlen(p)<<1), 35+10-l, p, 4, 10+16+128);
            minitext(160-(Bstrlen(p)<<1), 34+10-l, p, 8, 10+16+128);

            mgametext(160,54-l,"EDUKE 32 GAME PROGRAMMING",0,2+8+16);
            p = "Richard \"TerminX\" Gobeille";
            minitext(161-(Bstrlen(p)<<1), 55+10-l, p, 4, 10+16+128);
            minitext(160-(Bstrlen(p)<<1), 54+10-l, p, 8, 10+16+128);

            mgametext(160,73-l,"\"JFDUKE3D\" AND \"JFBUILD\" CODE",0,2+8+16);
            p = "Jonathon \"JonoF\" Fowler";
            minitext(161-(Bstrlen(p)<<1), 74+10-l, p, 4, 10+16+128);
            minitext(160-(Bstrlen(p)<<1), 73+10-l, p, 8, 10+16+128);

            mgametext(160,92-l,"BUILD ENGINE, \"POLYMOST\" RENDERER",0,2+8+16);
            mgametext(160,92+8-l,"NETWORKING, OTHER CODE",0,2+8+16);
            p = "Ken \"Awesoken\" Silverman";
            minitext(161-(Bstrlen(p)<<1), 93+8+10-l, p, 4, 10+16+128);
            minitext(160-(Bstrlen(p)<<1), 92+8+10-l, p, 8, 10+16+128);

            mgametext(160,119-l,"ADDITIONAL RENDERING FEATURES",0,2+8+16);
            ...
            mgametext(160,122-l,"LICENSE AND OTHER CONTRIBUTORS",0,2+8+16);
            {
                ...
                for (m=0,i=(totalclock/104)%numlines; m<6; m++,i++) {
                    if (i==numlines) i=0;
                    minitext(161-(Bstrlen(scroller[i])<<1), 117+10+10+8+4+(m*7)-l, (char*)scroller[i], 4, 10+16+128);
                    minitext(160-(Bstrlen(scroller[i])<<1), 116+10+10+8+4+(m*7)-l, (char*)scroller[i], 8, 10+16+128);
                }
            }

            p = "Visit http://code.google.com/p/erampage/ for news and updates";
            minitext(161-(Bstrlen(p)<<1), 148+10+10+10+10+4-l, p, 4, 10+16+128);
            minitext(160-(Bstrlen(p)<<1), 147+10+10+10+10+4-l, p, 8, 10+16+128);
        }
        break;

    case 0:
        c = (320>>1);
        rotatesprite(c<<16,28<<16,65536L/2,0,INGAMEDUKETHREEDEE,0,0,10,0,0,xdim-1,ydim-1);
    ...
    case 50:
        c = (320>>1);
        rotatesprite(c<<16,32<<16,65536L/2,0,INGAMEDUKETHREEDEE,0,0,10,0,0,xdim-1,ydim-1);
        if (GAME_RA)   // JBF 20030804
            rotatesprite((c+100)<<16,36<<16,65536L/2,0,PLUTOPAKSPRITE+2,(sintable[(totalclock<<4)&2047]>>11),0,2+8,0,0,xdim-1,ydim-1);
        x = M_Probe(c,67,16,7);
        switch (x) {
...
//            mgametext(160,160+9,"THIS OPTION INCURS A MOVEMENT DELAY",0,2+8+16);
    ...
    case 300:
        ...
        if (g_currentMenu >= 360 && g_currentMenu <= 369) {
            ...
            M_DisplaySaveGameList();
            rotatesprite((c+68+strlen(&ud.savegame[g_currentMenu-360][0])*4)<<16,(51+12*probey)<<16,4096L,0,SPINNINGNUKEICON+(((totalclock)>>3)%16),0,0,10,0,0,xdim-1,ydim-1);
            break;
        }
