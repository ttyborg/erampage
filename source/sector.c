...
//        sy = sprite[w].y;
    ...
    if ((picnum > MULTISWITCH2)&&(picnum <= MULTISWITCH2+3)) {
        switchpicnum = MULTISWITCH2;
    }
    ...
    while (i >= 0) {
        ...
        if (lotag == SLT) {
            ...
            if ((switchpicnum >= MULTISWITCH2) && (switchpicnum <=MULTISWITCH2+3)) {
                sprite[i].picnum++;
    ...
    for (i=numwalls-1; i>=0; i--) {
        ...
        if (lotag == wall[x].lotag) {
            ...
            if ((wall[x].picnum >= MULTISWITCH2) && (wall[x].picnum <=MULTISWITCH2+3)) {
                wall[x].picnum++;
    ...
    default:
        ...
        if ((sprite[sn].picnum == CIRCLESAW || sprite[sn].owner != i) && sprite[i].statnum != 4) {
            ...
            if (sprite[i].statnum == 10) {
                ...
                if (RX < 24 && sprite[sn].picnum == FIRELASER)
                    return;
...
void G_HandleSharedKeys(int32_t snum) {
    ...
    if (TEST_SYNC_KEY(sb_snum, SK_QUICK_KICK) && p->quick_kick == 0)
        if (p->curr_weapon != CROWBAR_WEAPON || p->kickback_pic == 0) {
            ...
            //            if(  ( p->weapon_pos == 0 || ( p->holster_weapon && p->weapon_pos == -9 ) ))
            {
                if (j == 10 || j == 11) {
                    ...
                    while ((k >= 0 && k < 10) || (GAME_RA && k == BUZSAW_WEAPON && (p->subweapon&(1<<BUZSAW_WEAPON)))) {       // JBF 20040116: so we don't select grower with v1.3d
                        if (k == BUZSAW_WEAPON) { // JBF: this is handling next/previous with the grower selected
                            ...
                            if (k == THROWSAW_WEAPON && (p->subweapon&(1<<BUZSAW_WEAPON)))    // JBF: activates grower
                                k = BUZSAW_WEAPON;                            // if enabled
                        }
                        ...
                        if (p->gotweapon[k] && p->ammo_amount[k] > 0) {
                            ...
                            if (k == THROWSAW_WEAPON && (p->subweapon&(1<<BUZSAW_WEAPON)))
                                k = BUZSAW_WEAPON;
                            j = k;
                            break;
                        } else   // JBF: grower with no ammo, but ALIENARMGUN with ammo, switch to shrink

                            if (k == BUZSAW_WEAPON && p->ammo_amount[BUZSAW_WEAPON] == 0 && p->gotweapon[THROWSAW_WEAPON] && p->ammo_amount[THROWSAW_WEAPON] > 0) {
                                j = THROWSAW_WEAPON;
                                p->subweapon &= ~(1<<BUZSAW_WEAPON);
                                break;
                            } else   // JBF: ALIENARMGUN with no ammo, but grower with ammo, switch to grow

                                if (k == THROWSAW_WEAPON && p->ammo_amount[THROWSAW_WEAPON] == 0 && p->gotweapon[BUZSAW_WEAPON] && p->ammo_amount[BUZSAW_WEAPON] > 0) {
                                    j = BUZSAW_WEAPON;
                                    p->subweapon |= (1<<BUZSAW_WEAPON);
                                    break;
                                }
                ...
                if (aGameVars[g_iReturnVarID].val.lValue == 0) {
                    ...
                    if (j == THROWSAW_WEAPON) {
                        ...
                        if (p->curr_weapon != BUZSAW_WEAPON && p->curr_weapon != THROWSAW_WEAPON) {
                            if (p->ammo_amount[BUZSAW_WEAPON] > 0) {
                                if ((p->subweapon&(1<<BUZSAW_WEAPON)) == (1<<BUZSAW_WEAPON))
                                    j = BUZSAW_WEAPON;
                                ...
                                p->subweapon &= ~(1<<BUZSAW_WEAPON);
                        } else if (p->curr_weapon == THROWSAW_WEAPON) {
                            p->subweapon |= (1<<BUZSAW_WEAPON);
                            j = BUZSAW_WEAPON;
                        } else
                            p->subweapon &= ~(1<<BUZSAW_WEAPON);
