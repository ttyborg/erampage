    ...
    for (x = 0; x<7; x++) {
        ...
        while (j >= 0) {
            ...
            if (x == 0 || x >= 5 || AFLAMABLE(sj->picnum)) {
                if (s->picnum != FIRELASER || (sj->cstat&257))
                    if (dist(s, sj) < r) {
                ...
                /*if (s->picnum == FIRELASER && sj->picnum != SHARK && (j == s->owner || sj->xrepeat < 24))
                {
                    j = nextj;
                    continue;
                }*/
...
int32_t A_MoveSprite(int32_t spritenum, const vec3_t *change, uint32_t cliptype) {
    ...
    if (bg) {
        ...
        if (sprite[spritenum].xrepeat > 60) {
            ...
            if ((ActorType[sprite[spritenum].picnum]&3))
    ...
    if (ActorExtra[sn].extra >= 0) {
        if (sprite[sn].extra >= 0) {
            ...
            if (npc->picnum == APLAYER) {
                if (ud.god && ActorExtra[sn].picnum != FIRELASER) return -1;
                ...
                if (j >= 0) {
                    if (npc->extra <= 0 && ActorExtra[sn].picnum != CIRCLESAW) {
                        npc->extra = 0;
                ...
                if (ActorExtra[sn].extra == 0)
                    if (ActorExtra[sn].picnum == FIRELASER && npc->xrepeat < 24)
                        return -1;

                npc->extra -= ActorExtra[sn].extra;
                if (npc->owner >= 0 && sprite[npc->owner].statnum < MAXSTATUS)
            ...
            if (p->newowner >= 0) { //Looking thru the camera
                ...
                if (ud.multimode > 1)
                    if (sprite[g_player[otherp].ps->i].extra > 0) {
                        if (s->yrepeat > 32 && sprite[g_player[otherp].ps->i].yrepeat < 16) {
...
DETONATE:
        ...
        if ((s->picnum > BOLT1) && (s->picnum <= BOLT1+2)) {
            switchpicnum = BOLT1;
        }
...
CLEAR_THE_BOLT:
            ...
            if (s->picnum==BOLT1+3) s->picnum=BOLT1;
            ...
            case COOLEXPLOSION1__STATIC:
                ...
                if (j != 0) {
                    ...
                    if ((j&49152) == 49152) {
                        ...
                        if (s->picnum == CIRCLESAW && sprite[j].pal == 1)
                        ...
                        if (s->picnum != RPG && s->picnum != CIRCLESAW && s->picnum != SPIT && (wall[j].overpicnum == MIRROR || wall[j].picnum == MIRROR)) {
                            k = getangle(
                                    wall[wall[j].point2].x-wall[j].x,
                                    wall[wall[j].point2].y-wall[j].y);
                            s->ang = ((k<<1) - s->ang)&2047;
                    ...
                    if (s->picnum != SPIT) {
                        ...
                        } else if (s->picnum == FIRELASER) {
                            A_Spawn(i,ALIENARMGUNEXPLOSION);
                            A_PlaySound(ALIENARMGUN_HIT,i);
                            A_RadiusDamage(i,g_ALIENARMGUNBlastRadius,0,0,0,0);
                        } else if (s->picnum != COOLEXPLOSION1 && s->picnum != CIRCLESAW && s->picnum != FIRELASER) {
                            k = A_Spawn(i,EXPLOSION2);
        ...
        case GREENSLIME__STATIC:
            ...
            if (t[0] == -4) { //On the player
                ...
                if ((TEST_SYNC_KEY(g_player[p].sync->bits, SK_FIRE) || (g_player[p].ps->quick_kick > 0)) && sprite[g_player[p].ps->i].extra > 0)
                    if (g_player[p].ps->quick_kick > 0 || (g_player[p].ps->curr_weapon != HANDREMOTE_WEAPON && g_player[p].ps->curr_weapon != DYNAMITE_WEAPON && g_player[p].ps->curr_weapon != POWDERKEG_WEAPON && g_player[p].ps->ammo_amount[g_player[p].ps->curr_weapon] >= 0)) {
                        for (x=0; x<8; x++) {
...
//*/
        ...
        case MORTER__STATIC:
            j = A_Spawn(i,(GAME_RA?FRAMEEFFECT1:FRAMEEFFECT1_13));
...
DETONATEB:
            ...
            if (t[6] == 3) {
                ...
                if (cansee(s->x,s->y,s->z-(8<<8),s->sectnum,g_player[p].ps->posx,g_player[p].ps->posy,g_player[p].ps->posz,g_player[p].ps->cursectnum))
                    if (g_player[p].ps->ammo_amount[DYNAMITE_WEAPON] < g_player[p].ps->max_ammo_amount[DYNAMITE_WEAPON]) {
                        ...
                        P_AddAmmo(DYNAMITE_WEAPON,g_player[p].ps,1);
                        A_PlaySound(DUKE_GET,g_player[p].ps->i);

                        if (g_player[p].ps->gotweapon[DYNAMITE_WEAPON] == 0 || s->owner == g_player[p].ps->i) {
                            /* P_AddWeapon(g_player[p].ps,DYNAMITE_WEAPON); */
                            if (!(g_player[p].ps->weaponswitch & 1) && *aplWeaponWorksLike[g_player[p].ps->curr_weapon] != HANDREMOTE_WEAPON)
                                P_AddWeaponNoSwitch(g_player[p].ps,DYNAMITE_WEAPON);
                            else P_AddWeapon(g_player[p].ps,DYNAMITE_WEAPON);
                        }
    ...
    while (i >= 0) {
        ...
        if ((s->picnum == MONEY+1) || (s->picnum == MAIL+1) || (s->picnum == PAPER+1))
            ...
            case FRAMEEFFECT1_13__STATIC:
                if (GAME_RA) goto BOLT;	// JBF: ideally this should never happen...
...
int32_t A_CheckSwitchTile(int32_t i) {
    ...
    if ((PN >= MULTISWITCH2) && (PN <=MULTISWITCH2+3)) return 1;
