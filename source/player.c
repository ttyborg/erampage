...
static int32_t A_FindTargetSprite(spritetype *s,int32_t aang,int32_t atwith) {
    int32_t gotALIENARMGUN,gotBUZSAWr;
    ...
    gotALIENARMGUN = (s->picnum == APLAYER && *aplWeaponWorksLike[g_player[s->yvel].ps->curr_weapon] == ALIENARMGUN_WEAPON);
    gotBUZSAWr = (s->picnum == APLAYER && *aplWeaponWorksLike[g_player[s->yvel].ps->curr_weapon] == THROWSAW_WEAPON);
            ...
            if (sprite[i].xrepeat > 0 && sprite[i].extra >= 0 && (sprite[i].cstat&(257+32768)) == 257)
                if (A_CheckEnemySprite(&sprite[i]) || k < 2) {
                    if (A_CheckEnemySprite(&sprite[i]) || PN == APLAYER /*|| PN == SHARK*/) {
                        ...
                        if (gotALIENARMGUN && sprite[i].xrepeat < 30) {
                            /*if (PN == SHARK)
                            {
                            ...
                            if (!(PN >= GREENSLIME && PN <= GREENSLIME+7))
                                continue;
                        }
                        if (gotBUZSAWr && sprite[i].pal == 1) continue;
                    }
...
DOSKIPBULLETHOLE:
        ...
        if (ProjectileData[atwith].workslike & PROJECTILE_RPG) {
            ...
            if (!(ProjectileData[atwith].workslike & PROJECTILE_BOUNCESOFFWALLS))
                ...
                else sprite[j].yvel = g_numBUZSAWBounces;
                //                sprite[j].xrepeat >>= 1;
                //                sprite[j].yrepeat >>= 1;
                sprite[j].zvel -= (2<<4);
            }
...
SKIPBULLETHOLE:
        ...
        case RPG__STATIC:
            ...
            if (atwith != CIRCLESAW)
                ...
                sprite[j].yvel = g_numBUZSAWBounces;
                sprite[j].xrepeat >>= 1;
                sprite[j].yrepeat >>= 1;
                sprite[j].zvel -= (2<<4);
            }
            ...
            else if (*aplWeaponWorksLike[g_player[p].ps->curr_weapon] == TEATGUN_WEAPON) {
                sprite[j].extra >>= 2;
        ...
        case ALIENARMGUN__STATIC:
            ...
            j = A_InsertSprite(sect,
                               ...
                               srcvect.z+(2<<8),FIRELASER,-16,28,28,sa,768,zvel,i,4);
...
static void G_DrawTileScaled(int32_t x, int32_t y, int32_t tilenum, int32_t shade, int32_t orientation, int32_t p) {
    ...
    rotatesprite(weapsc((orientation&256)?x:(x<<16))+((xoff-weapsc(xoff))<<16),
                 ...
                 weapsc(65536L/zoom),a,tilenum,shade,p,2|orientation,windowx1,windowy1,windowx2,windowy2);
}
...
static void G_DrawWeaponTile(int32_t x, int32_t y, int32_t tilenum, int32_t shade, int32_t orientation, int32_t p) {
    if (!ud.drawweapon)
        ...
        case SHOTGUN_WEAPON:
            rotatesprite(160<<16,(180+(g_player[screenpeek].ps->weapon_pos*g_player[screenpeek].ps->weapon_pos))<<16,scale(65536,ud.statusbarscale,100),0,g_currentweapon==BUZSAW_WEAPON?GROWSPRITEICON:WeaponPickupSprites[g_currentweapon],0,0,2,windowx1,windowy1,windowx2,windowy2);
            break;
        }
    }
}
...
void P_FireWeapon(DukePlayer_t *p) {
    ...
    if (aGameVars[g_iReturnVarID].val.lValue == 0) {
        ...
        if (aplWeaponWorksLike[p->curr_weapon][snum]!=CROWBAR_WEAPON &&
                aplWeaponWorksLike[p->curr_weapon][snum]!=BUZSAW_WEAPON&&
                aplWeaponWorksLike[p->curr_weapon][snum]!=SLINGBLADE_WEAPON)
        ...
        for (i=aplWeaponShotsPerBurst[p->curr_weapon][snum]-1; i > 0; i--) {
            if (aplWeaponFlags[p->curr_weapon][snum] & WEAPON_FIREEVERYOTHER) {
                ...
                if (aplWeaponFlags[p->curr_weapon][snum] & WEAPON_AMMOPERSHOT &&
                        aplWeaponWorksLike[p->curr_weapon][snum] != CROWBAR_WEAPON) {
        ...
        if (sprite[p->i].xrepeat < 16) {
            ...
            case CROWBAR_WEAPON:
                ...
                if (aGameVars[g_iReturnVarID].val.lValue == 0) {

                    {
                        ...
                        if ((*kb) < 16)
                            G_DrawTileScaled(weapon_xoffset+220-(p->look_ang>>1),
                                             looking_arc+250-gun_pos,KNEE+(*kb)/2,gs,o,pal);
                        else
                            G_DrawTileScaled(weapon_xoffset+160-(p->look_ang>>1),
                                             looking_arc+214-gun_pos,KNEE+7,gs,o,pal);
            ...
            case CROSSBOW_WEAPON:
                ...
                if (aGameVars[g_iReturnVarID].val.lValue == 0) {
                    ...
                    //*/
                    if ((*kb) < 16)
                        G_DrawWeaponTile(weapon_xoffset+164,(looking_arc<<1)+249-gun_pos,
                                         RPGGUN+((*kb)>>1),gs,o,pal);
                    else
                        G_DrawWeaponTile(weapon_xoffset+164,(looking_arc<<1)+249-gun_pos,
                                         RPGGUN,gs,o,pal);
            ...
            case SHOTGUN_WEAPON:
                ...
                if (aGameVars[g_iReturnVarID].val.lValue == 0) {
                    ...
                    //*/
                    if ((*kb) < 18)
                        G_DrawWeaponTile(weapon_xoffset+164,(looking_arc<<1)+235-gun_pos,
                                         SHOTGUN+((*kb)>>1),gs,o,pal);
                    else
                        G_DrawWeaponTile(weapon_xoffset+164,(looking_arc<<1)+235-gun_pos,
                                         SHOTGUN,gs,o,pal);
                }
            ...
            case RIFLE_WEAPON:
                ...
                if (aGameVars[g_iReturnVarID].val.lValue == 0) {
                    ...
                    default:
                        ...
                        /*
                        if (*kb > *aplWeaponFireDelay[RIFLE_WEAPON] && *kb < *aplWeaponTotalTime[RIFLE_WEAPON])
                        {
                            i = 0;
                        ...
                        if (*kb < *aplWeaponTotalTime[RIFLE_WEAPON]-4)
                        {
                            i = rand()&7;
                        ...
                        if (*kb > *aplWeaponFireDelay[RIFLE_WEAPON] && *kb < *aplWeaponTotalTime[RIFLE_WEAPON]) {
                            i = 0;
                            if (sprite[p->i].pal != 1) i = rand()&7;
                            G_DrawWeaponTile(i+weapon_xoffset-4+184-(p->look_ang>>1),i+looking_arc-((*kb)>>1)+233-gun_pos,
                                             CHAINGUN+2,gs,o,pal);
                        } else G_DrawWeaponTile(weapon_xoffset+178-(p->look_ang>>1),looking_arc+233-gun_pos,
                                                    CHAINGUN+1,gs,o,pal);
                        break;
            ...
            case CASUL_WEAPON:
                ...
                if (aGameVars[g_iReturnVarID].val.lValue == 0) {
                    ...
                    /*
                    if ((*kb) < *aplWeaponTotalTime[CASUL_WEAPON]+1)
                    {
                        static uint8_t kb_frames[] = { 0, 1, 2 };
                        int32_t l = 195-12+weapon_xoffset;

                        if ((*kb) == *aplWeaponFireDelay[CASUL_WEAPON])
                            l -= 3;
                    ...
                    else
                    {
                        ...
                        if ((*kb) < *aplWeaponReload[CASUL_WEAPON]-17)
                        {
                            guniqhudid = cw;
                            G_DrawWeaponTile(194-(p->look_ang>>1),looking_arc+230-gun_pos,FIRSTGUN+4,gs,o,pal);
                            guniqhudid = 0;
                        }
                        else if ((*kb) < *aplWeaponReload[CASUL_WEAPON]-12)
                        {
                        ...
                        else if ((*kb) < *aplWeaponReload[CASUL_WEAPON]-7)
                        {
                        ...
                        else if ((*kb) < *aplWeaponReload[CASUL_WEAPON]-4)
                        {
                        ...
                        else if ((*kb) < *aplWeaponReload[CASUL_WEAPON]-2)
                        {
                        ...
                        else if ((*kb) < *aplWeaponReload[CASUL_WEAPON])
                        {
                            guniqhudid = cw;
            ...
            case TEATGUN_WEAPON:
                ...
                if (aGameVars[g_iReturnVarID].val.lValue == 0) {
                    ...
                    /*
                    if ((*kb) < (*aplWeaponTotalTime[TEATGUN_WEAPON]+1) && (*kb) > 0)
                    {
                        ...
                        {
                            guniqhudid = cw;
                            G_DrawWeaponTile((cycloidy[*kb]>>1)+weapon_xoffset+268-(p->look_ang>>1),cycloidy[*kb]+looking_arc+238-gun_pos,TEATGUN+i,-32,o,pal);
                            guniqhudid = cw<<1;
                            G_DrawWeaponTile(weapon_xoffset+30-(p->look_ang>>1),looking_arc+240-gun_pos,TEATGUN,gs,o|4,pal);
                            guniqhudid = 0;
                        }
                        else
                        {
                            guniqhudid = cw<<1;
                            G_DrawWeaponTile(-(cycloidy[*kb]>>1)+weapon_xoffset+30-(p->look_ang>>1),cycloidy[*kb]+looking_arc+240-gun_pos,TEATGUN+i,-32,o|4,pal);
                            guniqhudid = cw;
                            G_DrawWeaponTile(weapon_xoffset+268-(p->look_ang>>1),looking_arc+238-gun_pos,TEATGUN,gs,o,pal);
                            guniqhudid = 0;
                        }
                    }
                    else
                    ...
                    //*/
                    if ((*kb) < 4)
                        G_DrawWeaponTile(weapon_xoffset+164,(looking_arc<<1)+190,
                                         TEATGUN+1,gs,o,pal);
                    else if ((*kb) < 8)
                        G_DrawWeaponTile(weapon_xoffset+164,(looking_arc<<1)+190,
                                         TEATGUN,gs,o,pal);
            ...
            case THROWSAW_WEAPON:
                ...
                if (aGameVars[g_iReturnVarID].val.lValue == 0) {
                    ...
                    if ((*kb) < (aplWeaponTotalTime[p->curr_weapon][snum]+1) && (*kb) > 0) {
                        ...
                        gun_pos -= 16;
                        guniqhudid = 0;
                        G_DrawWeaponTile(weapon_xoffset+210-(p->look_ang>>1),looking_arc+261-gun_pos,BUZSAW+2,-32,o,pal);
                        guniqhudid = cw;
                        G_DrawWeaponTile(weapon_xoffset+210-(p->look_ang>>1),looking_arc+235-gun_pos,BUZSAW+3+cat_frames[*kb%6],-32,o,pal);
                        guniqhudid = 0;
                    } else {
                        guniqhudid = cw;
                        G_DrawWeaponTile(weapon_xoffset+210-(p->look_ang>>1),looking_arc+261-gun_pos,BUZSAW,gs,o,pal);
                        guniqhudid = 0;
                    }
                }
                break;
            ...
            case ALIENARMGUN_WEAPON:
                ...
                if (aGameVars[g_iReturnVarID].val.lValue == 0) {
                    ...
                    if (((*kb) > 0) && ((*kb) < aplWeaponTotalTime[p->curr_weapon][snum])) {
                        ...
                        guniqhudid = cw<<1;
                        G_DrawWeaponTile(weapon_xoffset+222-(p->look_ang>>1),
                                         looking_arc+240-gun_pos,ALIENARMGUN+3+((*kb)&3),-32,
                                         o,0);
                        guniqhudid = cw;
                        G_DrawWeaponTile(weapon_xoffset+224-(p->look_ang>>1),
                                         looking_arc+240-gun_pos,ALIENARMGUN+1,gs,o,pal);
                        guniqhudid = 0;

                    } else {
                        guniqhudid = cw<<1;
                        G_DrawWeaponTile(weapon_xoffset+222-(p->look_ang>>1),
                                         looking_arc+240-gun_pos,ALIENARMGUN+2,
                                         16-(sintable[p->random_club_frame&2047]>>10),
                                         o,0);
                        guniqhudid = cw;
                        G_DrawWeaponTile(weapon_xoffset+224-(p->look_ang>>1),
                                         looking_arc+240-gun_pos,ALIENARMGUN,gs,o,pal);
                        guniqhudid = 0;
                    }
                }
                break;
            ...
            case SLINGBLADE_WEAPON:
                ...
                if (aGameVars[g_iReturnVarID].val.lValue == 0) {
                    ...
                    if ((*kb) < 14)
                        G_DrawWeaponTile(weapon_xoffset+164,(looking_arc<<1)+235-gun_pos,
                                         SLINGBLADE+((*kb)>>1),gs,o,pal);
                    else
                        G_DrawWeaponTile(weapon_xoffset+164,(looking_arc<<1)+235-gun_pos,
                                         SLINGBLADE+8,gs,o,pal);
                }
                break;
            ...
            case MOTORCYCLE_WEAPON:
                ...
                if (aGameVars[g_iReturnVarID].val.lValue == 0) {
                    ...
                    if ((*kb) < 18)
                        G_DrawWeaponTile(160,(looking_arc<<1)+180,
                                         MOTOHIT+((*kb)>>1),gs,o,pal);
                    else
                        G_DrawWeaponTile(160,(looking_arc<<1)+180,
                                         MOTOHIT,gs,o,pal);
                }
...
int16_t WeaponPickupSprites[MAX_WEAPONS] = { KNEE__STATIC, FIRSTGUNSPRITE__STATIC, SHOTGUNSPRITE__STATIC,
        CHAINGUNSPRITE__STATIC, RPGSPRITE__STATIC, HEAVYHBOMB__STATIC, ALIENARMGUNSPRITE__STATIC, TEATGUNSPRITE__STATIC,
        POWDERKEGSPRITE__STATIC, BUZSAWSPRITE__STATIC, HEAVYHBOMB__STATIC, ALIENARMGUNSPRITE__STATIC
                                           };
...
void P_AddWeaponNoSwitch(DukePlayer_t *p, int32_t weapon) {
    ...
    if (p->gotweapon[weapon] == 0) {
        ...
        if (weapon == ALIENARMGUN_WEAPON)
            p->gotweapon[BUZSAW_WEAPON] = 1;
    }
...
void P_CheckTouchDamage(DukePlayer_t *p,int32_t j) {
    int vel = ((p->posx-p->oposx)*(p->posx-p->oposx) + (p->posy-p->oposy)*(p->posy-p->oposy))/1000;
    if ((j&49152) == 49152) {
        j &= (MAXSPRITES-1);
        ...
        if (p->curr_weapon == MOTORCYCLE_WEAPON || p->curr_weapon == BOAT_WEAPON) {
            ...
            if (vel > 10) {
                ActorExtra[j].picnum = MOTOHIT;
                ActorExtra[j].extra = vel;
            }
        } else if (sprite[j].picnum == CACTUS) {
            if (p->hurt_delay < 8) {
                sprite[p->i].extra -= 5;
...
//    if ((sb_snum&(1<<2)))
    ...
    if (psectlotag == 2) {
        ...
        if (p->posz < (fz-(i<<8))) { //falling
            ...
            if (!TEST_SYNC_KEY(sb_snum, SK_JUMP) && !TEST_SYNC_KEY(sb_snum, SK_CROUCH) &&
                ...
                p->poszv += (playerGravity+80); // (TICSPERFRAME<<6);
                if (p->poszv >= (4096+2048)) p->poszv = (4096+2048);
                if (p->poszv > 2400 && p->falling_counter < 255) {
                    p->falling_counter++;
                    if (p->falling_counter >= playerMaxFallTime/2 && p->scream_voice == -1)
    ...
    if (p->fist_incs ||
            ...
            p->access_incs > 0 ||
            p->knee_incs > 0 ||
            (*aplWeaponWorksLike[p->curr_weapon] == POWDERKEG_WEAPON &&
             *kb > 1 &&
             *kb < 4)) {
        doubvel = 0;
        p->posxv = 0;
    ...
    if (p->posxv || p->posyv || g_player[snum].sync->fvel || g_player[snum].sync->svel) {
        ...
        if (p->on_ground && (TEST_SYNC_KEY(sb_snum, SK_CROUCH) ||
                             (*kb > 10 && aplWeaponWorksLike[p->curr_weapon][snum] == CROWBAR_WEAPON))) {
...
HORIZONLY:
    ...
    if (p->show_empty_weapon > 0) {
        ...
        if (p->show_empty_weapon == 0 && (p->weaponswitch & 2) && p->ammo_amount[p->curr_weapon] <= 0) {
            if (p->last_full_weapon == BUZSAW_WEAPON)
                p->subweapon |= (1<<BUZSAW_WEAPON);
            else if (p->last_full_weapon == ALIENARMGUN_WEAPON)
                p->subweapon &= ~(1<<BUZSAW_WEAPON);
...
SHOOTINCODE:
    if (TEST_SYNC_KEY(sb_snum, SK_HOLSTER)) { // 'Holster Weapon
        ...
        if (aGameVars[g_iReturnVarID].val.lValue == 0) {
            if (*aplWeaponWorksLike[p->curr_weapon] != CROWBAR_WEAPON) {
    ...
    if (shrunk || p->tipincs || p->access_incs)
        ...
        if (aplWeaponWorksLike[p->curr_weapon][snum] == DYNAMITE_WEAPON) {
            ...
            if ((*kb) >= aplWeaponTotalTime[p->curr_weapon][snum]) {
                ...
                if ((p->ammo_amount[DYNAMITE_WEAPON] > 0) &&
                        Gv_GetVarByLabel("PIPEBOMB_CONTROL", PIPEBOMB_REMOTE, -1, snum) == PIPEBOMB_REMOTE) //? !!!!!!!!!!!!1
            ...
            if (aplWeaponFlags[p->curr_weapon][snum] & WEAPON_CHECKATRELOAD) {
                if (aplWeaponWorksLike[p->curr_weapon][snum] == POWDERKEG_WEAPON) {
                    if ((*kb) >= aplWeaponTotalTime[p->curr_weapon][snum]) {
                        (*kb) = 0;
                        P_CheckWeapon(p);
                        p->weapon_pos = -9;
                    ...
                    P_CheckWeapon(p);
            } else if (aplWeaponWorksLike[p->curr_weapon][snum]!=CROWBAR_WEAPON && *kb >= aplWeaponFireDelay[p->curr_weapon][snum])
                P_CheckWeapon(p);
            ...
            if ((*kb) >= aplWeaponTotalTime[p->curr_weapon][snum]) {
                if (/*!(aplWeaponFlags[p->curr_weapon][snum] & WEAPON_CHECKATRELOAD) && */ p->reloading == 1 ||
                    ...
                    if (aplWeaponFlags[p->curr_weapon][snum] & WEAPON_AUTOMATIC &&
                            (aplWeaponWorksLike[p->curr_weapon][snum]==CROWBAR_WEAPON?1:p->ammo_amount[p->curr_weapon] > 0)) {
                        if (TEST_SYNC_KEY(sb_snum, SK_FIRE)) {
...
//                                P_DoWeaponSpawn(p);
                        ...
                        if (aplWeaponFlags[p->curr_weapon][snum] & WEAPON_RESET &&
                                ...
                                ((aplWeaponWorksLike[p->curr_weapon][snum] == CROWBAR_WEAPON) || p->ammo_amount[p->curr_weapon] > 0)) {
                            if (TEST_SYNC_KEY(sb_snum, SK_FIRE)) *kb = 1;
                            else *kb = 0;
                        }
                    } else {
...
int32_t getspritescore(int32_t snum, int32_t dapicnum) {
    ...
    case BUZSAWAMMO__STATIC:
        if (g_player[snum].ps->ammo_amount[THROWSAW_WEAPON] < g_player[snum].ps->max_ammo_amount[THROWSAW_WEAPON]) return(10);
        return(1);
    case AMMO__STATIC:
        if (g_player[snum].ps->ammo_amount[CASUL_WEAPON] < g_player[snum].ps->max_ammo_amount[CASUL_WEAPON]) return(10);
        return(1);
    case BATTERYAMMO__STATIC:
        if (g_player[snum].ps->ammo_amount[RIFLE_WEAPON] < g_player[snum].ps->max_ammo_amount[RIFLE_WEAPON]) return(20);
        return(1);
    case TEATGUNAMMO__STATIC:
        if (g_player[snum].ps->ammo_amount[TEATGUN_WEAPON] < g_player[snum].ps->max_ammo_amount[TEATGUN_WEAPON]) return(25);
        return(1);
    case RPGAMMO__STATIC:
        if (g_player[snum].ps->ammo_amount[CROSSBOW_WEAPON] < g_player[snum].ps->max_ammo_amount[CROSSBOW_WEAPON]) return(50);
        return(1);
    case ALIENBLASTERAMMO__STATIC:
        if (g_player[snum].ps->ammo_amount[ALIENARMGUN_WEAPON] < g_player[snum].ps->max_ammo_amount[ALIENARMGUN_WEAPON]) return(10);
        return(1);
    case HBOMBAMMO__STATIC:
        if (g_player[snum].ps->ammo_amount[DYNAMITE_WEAPON] < g_player[snum].ps->max_ammo_amount[DYNAMITE_WEAPON]) return(30);
        return(1);
...
void computergetinput(int32_t snum, input_t *syn) {
    ...
    if ((g_player[goalplayer[snum]].ps->dead_flag == 0) &&
        ...
        if ((p->curr_weapon == DYNAMITE_WEAPON) && (!(rand()&7)))
            syn->bits &= ~BIT(SK_FIRE);
        ...
        if (sprite[g_player[goalplayer[snum]].ps->i].yrepeat < 16) {
            fightdist = 0;
            syn->bits &= ~BIT(SK_FIRE);
        }
